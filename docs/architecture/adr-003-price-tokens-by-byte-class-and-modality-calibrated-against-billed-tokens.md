...
(a) **Text is priced per byte class, not by one flat divisor.** One linear pass counts ASCII and non-ASCII bytes separately (a byte below 0x80 is one character; a non-ASCII character is two to four bytes, so a single divisor misprices whichever class it was not fitted to). Two divisors convert the counts to tokens — 2.55 bytes per ASCII token, 3.85 per non-ASCII token — plus 3 tokens of framing per message, matching OpenAI's cookbook. 3.85 bytes per token is 1.93 *characters* per token for two-byte Cyrillic, which independently matches the 1.5-2 chars/token o200k is documented to achieve on Russian. Worst-case error over the corpus fell from 39% to 3.7%.
